            ...
            for arch in arch_list:
                if "." in arch.path:
                    ...
                    shutil.unpack_archive(
                        arch_name, rf"{main_path}\{direct}\{folder_for_unpack}"
                    )

                    # Додаємо перевірки перед перейменуванням
                    if os.path.exists(arch_name):
                        if not os.path.exists(
                            f"{main_path}\\{direct}\\{folder_for_unpack}"
                        ):
                            os.mkdir(f"{main_path}\\{direct}\\{folder_for_unpack}")
                        os.rename(
                            arch_name,
                            f"{main_path}\\{direct}\\{folder_for_unpack}\\{name_for_move}",
                        )
                    else:
                        print(f"File {arch_name} does not exist!")
    return print("Ok!")
...
#     except IndexError:
#         return "Не вказано адресу теки для сортyвання."
...
def main(folder_path):
    ...
    if not folder_path.exists():
        return f'Теки з назвою "{folder_path}" не знайдено.'

    get_sort(folder_path)

    return "Все гаразд, сортування завершено!"


if __name__ == "__main__":
    path = input("Ведіть адресу теки для сортування: ")
    print(main(path))
